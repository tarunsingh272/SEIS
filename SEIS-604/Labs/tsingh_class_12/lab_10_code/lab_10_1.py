# complete code for running bfs on word ladder

# graph classes not imported but just included below
...
class Vertex:
    ...
    def __str__(self) -> str:
        ...
        return f"{self._key:^8}|{self._color:^8}|{self._distance:^8}"
...
def bfs(start,stop,g):
    start.distance = 0
    start.previous = None
    vert_queue = Queue()
    vert_queue.enqueue(start)
    while vert_queue.size() > 0:
        print (vert_queue) # ; input(">>>")
        ...
        for neighbor in current.get_neighbors():
            ...
            if neighbor.key == stop.key:
                print ("found a path!")
                print ("ending at:", neighbor.key)
                node = neighbor
                print ("now tracing back to: ", start.key)
                path = []
                node = stop # g.get_vertex(neighbor.key)
                while node:
                    path.append(node)
                    node = node.previous
                src = start.key
                dst = stop.key
                print(
                    f"Path from {g.get_vertex(src).key} "
                    + f"to {g.get_vertex(dst).key} "
                    + f"({g.get_vertex(dst).distance}): "
                    + f"{' '.join(vertex.key for vertex in reversed(path))}"
                )# while node.previous:
                #     print (node.key)
                #     node = node.previous
...
def traverse(g, src: "Vertex", dst: "Vertex") -> None:
    """Traverse a graph"""
    path = []
    current = g.get_vertex(dst)
    while current:
        path.append(current)
        current = current.previous
    print(
...
def main():
    ...
    # build a file with 4-letter words from words.txt

    fvar = open ("words.txt","r")
    wvar = open ("four_letters.txt","w")
    four_letter_words = []

    for word in fvar:
        if len(word)==5: # includes \n
            wvar.write(word)

    fvar.close()
    wvar.close()

    g = build_graph("four_letters.txt")

    # now do bfs after creating 'FOIL' Vertex

    # start = g.get_vertex('food')
    # stop = g.get_vertex('wise')

    start = g.get_vertex('sage')
    stop = g.get_vertex('sage')
